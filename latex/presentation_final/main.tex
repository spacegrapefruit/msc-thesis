...
\begin{frame}{Context \& Problem Statement}
  \begin{itemize}
    \item \textbf{Text-to-Speech (TTS)} systems (also known as speech synthesis) are widely used in virtual assistants, navigation, visual aids, and other applications.
    ...
    \item The \textit{Liepa-2} corpus contains 939 hours of annotated Lithuanian speech,
          but it is \textbf{fragmented} across 2,621 speakers.
    \item \textbf{Multi-speaker TTS} is a promising solution to leverage such data.
    \item \textbf{Question:} What is the optimal strategy for sampling multi-speaker data to maximize synthesis quality?
  \end{itemize}
\end{frame}
...
\begin{frame}{Aim \& Hypothesis}
  ...
  \vspace{1em}
  \textbf{Hypothesis:}
  Data \textit{depth} is the critical factor. Synthesis quality will degrade as the number of speakers increases if the total training duration remains constant.
\end{frame}
...
\begin{frame}{Acoustic Model Architectures}
  \begin{columns}[T]
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}
        ...
        \item 21 native Lithuanian listeners
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Results: Speaker-Dependent Naturalness}
  ...
  \begin{block}{Observations}
    ...
    \vspace{0.5em}
    \textbf{Qualitative analysis} revealed noticeable muffling and reverberation in MS052,
    VP131 recordings, pointing to data quality issues.
  \end{block}
\end{frame}
...
\begin{frame}{Conclusions}
  \begin{enumerate}
    ...
    \item Audio quality (SNR, reverberation) and/or speaker characteristics have a strong
          impact on synthesis naturalness.
  \end{enumerate}
\end{frame}
...
\begin{frame}{Recommendations \& Future Work}
  ...
  \begin{itemize}
    ...
    \item Multi-speaker TTS models can be successfully trained using sparse data with 7.5
          min/speaker.
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item Test the ``Breadth vs. Depth'' trade-off at higher budgets (e.g., 100+ hours).
...
\begin{frame}{Appendix 2: Latin Square Design Example}
  \begin{figure}
    \centering
