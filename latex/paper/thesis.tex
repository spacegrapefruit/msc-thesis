...
for the visually impaired~\cite{isewon2014design, taylor2009text}, providing a
voice for the non-verbal, and facilitating access to
...
remarkably natural speech, these models lack any explicit linguistic prior
knowledge, making them heavily reliant on large amounts of high-quality
training data to learn implicit mappings from text to speech. The common
...
% TODO multi-speaker TTS requirements (hours)

% In low-resource scenarios, this poses a significant risk: if the
% model fails to learn robust attention alignments due to data sparsity, the
% resulting synthesis suffers from critical failure modes such as babbling,
% skipping words, or endless repetition~\cite{ren2019fastspeech}. Consequently,
% investigating the composition of data required to stabilize these alignment
% mechanisms is crucial for adapting E2E models to languages lacking `Gold
% Standard'' corpora.
...
greater than twice the highest frequency present in the signal. Frequencies in
the range between 300~Hz and 3400~Hz contribute most to human speech
intelligibility and recognition~\cite{jothilakshmi2016large}. However, for
highly natural-sounding synthesized speech, higher sampling rates are
preferred. In TTS applications, common sampling rates for audio are 22.05~kHz
and 24~kHz, which can capture frequencies up to approximately 11~kHz and
12~kHz, respectively.
...
precision of the representation. Similar to sampling rate, there is a trade-off
between audio quality and data size. Common bit depths for audio are 16-bit and
...
In the context of neural TTS systems, which typically operate on
frequency-domain representations like Mel-spectrograms (discussed below),
pre-emphasis is a common audio pre-processing step applied to the raw waveform.
Natural speech signals tend to have more energy in the lower frequencies, with
a gradual drop-off towards higher frequencies (typically around -6~dB per
octave). This spectral tilt can lead to models focusing disproportionately on
...
While the spectrogram provides a detailed frequency analysis, it uses a linear
frequency scale. However, the human ear does not perceive frequencies linearly
--- it is more sensitive to pitch differences at lower frequencies than higher
ones. To mimic this perceptual characteristic, the Mel
scale~\cite{stevens1937scale} maps linear frequency \( f \) (in Hz) to a
perceptual scale \( m \) (in Mels) using the following formula:
...
triangular filters to the (magnitude) spectrogram obtained from the STFT\@.
...
(which will be discussed in section~\ref{sec:vocoder}).

\subsection{Linguistic Representation (Text Pre-processing)}
...
audio, in practice, pre-processing the text makes the model convergence easier
and improves the quality of the synthesized speech.
...
``read'' /riːd/ vs.\ ``read'' /rɛd/, depending on the tense).
...
determine the meaning of monographic words. Examples are shown in
...
From a neural modeling perspective, the high inflectional nature of Lithuanian
increases the effective sequence length of the input. Autoregressive TTS models
with location-sensitive attention, such as Tacotron~2, are known to degrade in
performance as input sequences grow longer~\cite{battenberg2020location}. The
complex G2P mapping inherent in Lithuanian free stress further complicates the
learning objective. The model must reserve capacity to memorize stress patterns
for specific inflections rather than just generalizing on acoustic features.
This suggests that for Lithuanian, the stability of the alignment mechanism is
not only dependent on the amount of available data, but is also affected by the
...
simple word-dictionary lookup, which does not take into account the context of
the word. Thus, it suggests multiple possible accentuation variants for
homographs, leaving it up to the user to select the correct one.
...
accentuation variants for a word, no stress marks will be added, forcing the
TTS model to rely on the context to resolve the ambiguity, rather than
providing potentially incorrect explicit labels.
...
preventing overfitting to noise and idiosyncrasies of individual speakers, and
leading to a more robust internal representation. However, in low depth
scenarios, the model may struggle to converge on stable speaker identities,
resulting in an ``average'' voice that lacks distinctiveness and naturalness.
In contrast, datasets with fewer speakers but high duration per speaker allow
the model to capture fine-grained prosodic details specific to those voices,
potentially achieving lower generalization, but higher fidelity for the
speakers present in the training set. It remains an open question whether the
regularization benefit of speaker diversity outweighs the noise introduced by
insufficient speaker profiling.
...
vector space~\cite{mikolov2013efficient}. Just as similar words map to nearby
points, in the TTS context, input characters that sound similar (e.g., `p' and
`b') often acquire similar embedding representations during training.
...
specifically for processing sequential data. Unlike Feedforward networks, RNNs
possess an internal state (memory) that allows information to persist across
time steps. For every element in the input sequence, the network performs the
same task, with the output depending on both the current input and the previous
computations. This makes them naturally suited for speech synthesis, where the
current acoustic frame is highly dependent on the preceding ones.
...
Bi-directional RNNs (Bi-LSTMs) are often used in the encoder to capture
...
generation. Rather than compressing the entire input into a single fixed-length
vector, attention creates a context vector that is a weighted sum of the input
states, where the weights represent the ``relevance'' of each input token to
the current output step.

In TTS, the alignment must be monotonic (the model should read text from left
to right without skipping back and forth) and continuous. Standard
content-based attention can fail in this regard, leading to issues like
skipping words or repeating phrases. To solve this, TTS systems typically use
\textit{Location-Sensitive Attention}~\cite{chorowski2015attention}. This
variant adds the previous alignment history as an input to the attention
scoring mechanism, effectively allowing the model to know ``where it was'' in
the text at the last time step, and ensuring a stable and monotonic progression
...
Figure~\ref{fig:tacotron2_architecture}.

The architecture has three main components, an encoder, an attention mechanism,
and a decoder.
...
token prediction. Modern implementations of Tacotron~2 often include additional
loss terms, such as the spectral similarity index (SSIM) loss, and the ``guided
...
applies flow-based generative models (Normalizing Flows), which allow for
complex distributions to be modeled by transforming a simple distribution
through a series of invertible mappings. Unlike Tacotron~2, Glow-TTS generates
the entire Mel-spectrogram in parallel, significantly speeding up inference.

A distinguishing feature of Glow-TTS is its ability to learn alignment
internally, treating it as a latent variable. This eliminates the dependency on
external aligners or priors often required by early parallel models, or
original implementations of models like FastPitch~\cite{lancucki2020fastpitch}.
To achieve this, Glow-TTS employs Monotonic Alignment Search (MAS) to find the
most probable monotonic path between the input text and the target
Mel-spectrogram, maximizing the log-likelihood of the data. Architecturally,
the decoder uses a stack of normalizing flows --- specifically invertible $1
      \times 1$ convolutions and affine coupling layers --- to transform a simple
prior distribution (conditioned on the text encoder outputs) into the complex
distribution of Mel-spectrograms.
...
% TODO
% In the context of the proposed low-resource experiments, the distinction
% between Tacotron~2's soft attention and Glow-TTS's Monotonic Alignment Search
% (MAS) is critical. Tacotron~2 must learn to attend from scratch, which is
% notoriously data-inefficient and prone to collapse on small datasets.
% Conversely, Glow-TTS enforces a hard monotonic constraint. Theoretically, this
% inductive bias should make Glow-TTS significantly more data-efficient, as it
% restricts the search space of possible alignments. This thesis tests whether
% this architectural hardness allows Glow-TTS to produce intelligible speech in
% the extreme low-data regime (7.5 minutes per speaker) where the soft attention
% of Tacotron~2 is hypothesized to fail.
...
highly efficient and capable of Real-Time Factor significantly less than 1.0
...
speaker identity. In multi-speaker architectures, the network is conditioned on
a speaker embedding vector. The model learns a shared representation of
phonetics (how text maps to sound generally) while using this additional input
--- the speaker embedding --- to adjust the timbre and prosodic characteristics
specific to a target voice.
...
In this work, learnable speaker embeddings are used for both Tacotron~2 and
Glow-TTS models. While external embeddings allow for zero-shot capabilities,
learnable embeddings were chosen. The primary reason is that this study focuses
on synthesis for a known set of speakers, where learnable embeddings typically
outperform pre-computed vectors. They allow each model to learn a specialized
latent representation for each voice that reflects the features most relevant
for each model architecture, rather than features used specifically to
...
% TODO
% The choice of learnable embeddings over pre-computed vectors (e.g., x-vectors)
% is specifically motivated by the fragmented nature of the Liepa~2 corpus.
% Pre-computed embeddings rely on the assumption that the reference audio is of
% sufficient duration and stability to extract a text-independent speaker
% signature. In the target dataset, where some speakers may have limited,
% variable-quality samples, an external encoder might produce noisy or
% inconsistent embeddings. By using learnable embeddings, the TTS model can
% optimize the speaker vector to maximize the likelihood of the specific training
% samples available, effectively ``overfitting'' the speaker identity to the
% available data, which is a desirable trait when the goal is to maximize quality
% for a closed set of speakers rather than zero-shot generalization.
...
As discussed in \textbf{Deep learning for end-to-end TTS}, there is a trade-off
between the \textit{breadth} and \textit{depth} of the training data.
...
speech quality, including spectral similarity (MCD), pitch accuracy ($F_0$
RMSE), and perceived audio quality (PESQ).
...
while the remaining 61~hours consist of non-speech noise segments. The
...
The dataset was obtained from the authors upon request for research purposes.
...
Analysis reveals that the entire Liepa~2 corpus contains 939~hours of
transcribed speech. Of this, spontaneous speech constitutes under 9\%
(84~hours), while read speech makes up the majority (855~hours). Spontaneous
speech is more challenging for TTS training due to its inconsistent
pronunciation, disfluencies, and variable prosody. Read speech, on the other
hand, is generally more suitable for TTS training due to its consistent
...
recordings. It was decided to focus on read speech from studio, dictaphone, and
audiobook sources to ensure higher quality and consistency in the training
data.
...
adult speakers (VCTK dataset~\cite{veaux2019cstr}) and will be used for
waveform generation. Speakers in the 0--17 age group constitute a small
fraction (76 hours, 8\%) of the total corpus duration, so their exclusion is
not expected to significantly impact the available training data.
...
To summarize, the filtering criteria applied to the Liepa~2 dataset are as
follows:
...
After applying these filters, the resulting dataset contains 723 hours of read
...
ensure fair comparison across experiments. The configurations are defined in
Table~\ref{tab:data_subsets} and visually represented in
...
speaker) was held out for validation and testing. The durations listed in
Table~\ref{tab:data_subsets} represent the training partition only.
...
This nesting strategy ensures that the `core' 30 speakers are present in every
model. This enables a direct, side-by-side comparison of synthesis quality for
...
\begin{enumerate}
      ...
      \item \textbf{Accentuation:} Raw text was processed using \textbf{Kirčiuoklis}~\cite{kirciuoklis} for automatic stress assignment. The stress marks were decomposed into separate characters (`´' for acute, ``' for grave, `~' for circumflex) and inserted immediately after the accented grapheme. Ambiguous homographs were left unaccentuated, relying on the model to infer prosody from context.
...
using the parameters in Table~\ref{tab:audio_params}.
...
\begin{table}[htbp]
      \centering
      \caption{Tacotron~2 with DCA architecture details.}\label{tab:tacotron_arch}
      \begin{tabular}{lr}
            \toprule
            \textbf{Component}  & \textbf{Configuration}                               \\
            \midrule
            Text Encoder        & 3-layer convolution + bidirectional-LSTM (512 units) \\
...
with MAS\@. The backbone of the model is a Transformer encoder and a flow-based
decoder. The alignment between text and spectrogram frames is learned via
unsupervised Soft-DTW (Dynamic Time Warping), eliminating the need for external
aligners. The duration predictor is an explicit 1D-convolutional network.

% TODO Soft-DTW? review this entire section

% TODO expand architecture according to the paper
...
To enable multi-speaker synthesis, speaker identity was provided via learnable
embeddings.
...
the attention mechanism to factor in speaker identity when generating each
Mel-spectrogram frame.

In Glow-TTS, the embedding conditions the flow-based decoder. It is used to
predict the distribution (mean and variance) of the latent variables for each
speaker, effectively shifting the latent space to match the target voice
identity before the normalizing flow transforms it into a spectrogram.
...
deployment to the evaluation web app.
...
used instead after empirical testing. The main hyperparameters are shown in
Table~\ref{tab:tacotron_config}.
...
convergence. The configuration is shown in Table~\ref{tab:glow_config}.
...
The loss function was a combination of NLL loss for the flow-based decoder, and
MSE for the duration predictor.
...
A screenshot of the web application interface is shown in
Figure~\ref{fig:mos_app}.
...
Table~\ref{tab:objective_results} summarizes the MCD and F0~RMSE on the
held-out test set.
...
Mel-spectrogram comparison Figure~\ref{fig:spectrogram_comparison}, Tacotron~2
...
ground truth on a 5-point MOS scale. The results are presented in
Table~\ref{tab:mos_results}.
...
the average MOS per speaker across all models.
...
The experimental results only weakly support the hypothesis that data depth is
more important than breadth for multi-speaker TTS quality under a fixed budget.

The impact of data composition was subtle. The balanced strategy (60 speakers
with 22.5 minutes each) yielded the highest subjective ratings for both
architectures. On the other hand, the low-depth strategy (180 speakers with 7.5
minutes each) resulted in the lowest scores for both architectures. However,
the difference was marginal and only statistically significant for Glow-TTS\@.
This suggests that the models are relatively insensitive to the exact data
composition within the tested range, when the total data volume remains
constant.

Objective metrics such as F0 RMSE and MCD similarly exhibited minor variations
across the data strategies, with no clear trend indicating a substantial
performance degradation for higher speaker counts.

Despite the low data volume per speaker in the 180-speaker subset, both
architectures successfully converged, contradicting concerns that low-depth
settings would cause training issues. This indicates that 7.5 minutes of data
per speaker is above the `critical threshold' required to learn stable
alignments and synthesize intelligible speech for Lithuanian.

Tacotron~2 architecture consistently outperformed Glow-TTS across all data
configurations. Tacotron~2 achieved a peak MOS score of \textbf{3.12} in the
60-speaker setup, while Glow-TTS lagged behind with a maximum MOS of
\textbf{2.17}. Qualitatively, listeners noted that Tacotron~2 produced more
natural and expressive speech, while Glow-TTS outputs were often described as
robotic and ``scratchy''. It is unclear what specific architectural, language,
or dataset features led to this gap, but it may be hypothesized to Glow-TTS
lacking an explicit pitch predictor.

Finally, speaker-specific analysis revealed that certain speakers consistently
yielded significantly higher synthesis quality across models and data
strategies, indicating that speaker characteristics and data quality play a
crucial role in TTS performance. This variability implies that for future
low-resource datasets, automated filtering based on acoustic metrics (SNR,
reverberation) or predicted MOS may yield higher gains than simply increasing
...
findings reveal that modern architectures are robust to high data fragmentation
(up to 180 speakers) under a fixed training budget. This insight can inform
data collection strategies for low-resource languages.
...
Firstly, the experiments were conducted solely on read speech from the
...
may not reflect performance at other data scales, such as very low-resource
(e.g., under 5 hours) or high-resource (e.g., 100+ hours) scenarios.

Additionally, only two TTS architectures were evaluated. While Tacotron~2 and
Glow-TTS are popular TTS models, other architectures (e.g., VITS, FastSpeech~2)
may exhibit different sensitivities to data composition.

Finally, the subjective evaluation relied on a sample of 21 raters. While the
confidence intervals were sufficient to establish the performance gaps between
architectures and speakers, a larger-scale listening test would provide higher
statistical power to detect the subtle effects of data composition strategies.
...
Firstly, extending the study to include more TTS architectures, such as VITS or
FastSpeech~2, would provide a broader understanding of how different model
types respond to data composition strategies, and potentially identify
architectures more robust to low-depth settings.

Secondly, future work could investigate methods for automated selection of
`TTS-friendly' speakers. Given the significant quality variability observed
between speakers (regardless of data strategy or model architecture),
developing techniques to automatically identify high-quality speakers or
utterances could yield greater improvements than manual data curation or simply
increasing dataset size.

Replicating the experiments on different languages and datasets would help
validate the cross-lingual generalizability of the findings. Additionally,
exploring a wider range of data budgets could provide insights into scaling
effects. For instance, testing with a 10-hour or 50-hour budget could reveal
different trade-offs.

The evaluation could be expanded to include intelligibility metrics (e.g., word
error rate from ASR systems) to assess how data composition impacts not just
naturalness, but also clarity of the synthesized speech.
...
% TODO Examples are also provided for ChatGPT citation, both in general~\cite{chatgpt_bendrai} and for a specific conversation~\cite{chatgpt_pokalbis}.
