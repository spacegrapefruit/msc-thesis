...
Before diving into the specifics of TTS systems and data selection strategies,
it is important to establish the broader context of audio processing,
text-to-speech synthesis, and its evolution over time.

\subsection{Digital signal processing fundamentals}

\subsubsection{Digital representation of audio}

In the physical world, speech, or sound in general, is a continuous pressure
wave that propagates through a medium, such as air. Key properties of sound
waves include frequency (pitch), amplitude (loudness), and phase.
...
sampling rate (or sample frequency), measured in Hertz. According to the
Nyquist-Shannon~\cite{shannon1949communication} sampling theorem, accurate
reconstruction of a continuous signal requires a sampling rate that is at least
twice the highest frequency present in the signal. The human audible range is
typically considered to be from 20~Hz to 20~kHz, with most speech information
concentrated between 100~Hz and 8~kHz.~\cite{rabiner2010theory} Thus, in
text-to-speech applications, common sampling rates for audio are 22.05~kHz and
24~kHz, which can capture frequencies up to approx. 11~kHz and 12~kHz,
respectively.
...
signal, and \( \alpha \) is the pre-emphasis coefficient (typically between 0.9
and 1.0).
...
% TODO: maybe pre-emphasis not needed anymore with modern neural TTS?
...
short frames (e.g., 25~ms) with a certain overlap (e.g., 10~ms) between frames.
...
The human ear does not perceive frequencies linearly - it is more sensitive to
lower frequencies than higher ones. To mimic this perceptual characteristic,
...
centuries opened new possibilities for speech synthesis. Homer Dudley's
Voder~\cite{klatt1987review} (Voice Operation Demonstrator), unveiled in 1937,
represented the first successful electronic recreation of human speech. This
invention allowed a trained human operator to synthesize intelligible speech by
...
% TODO Specific challenges in Lithuanian (inflection, accentuation).
...
% TODO add about Kirčiuoklis accentuation tool

% TODO In the absence of a high-quality G2P converter for Lithuanian, this thesis
% focuses on grapheme-based TTS synthesis.

% TODO Lithuanian, for example, has a rich inflectional morphology and complex
% accentuation patterns that can make grapheme-to-phoneme mapping non-trivial.
...
% Transfer learning approaches~\cite{jia2018transfer} have demonstrated effective
% methods for adapting speaker verification models to multispeaker TTS synthesis,
% enabling better speaker representation and voice quality.
...
% \textbf{Global Style Tokens (GST):} GSTs~\cite{wang2018style} provide an unsupervised method for learning style and prosody representations in TTS. A reference encoder processes a reference audio sample to extract a style embedding, which is then used to attend over a set of learned style tokens.
...
neural network. This approach promised to eliminate the need for hand-crafted
...
% The sequence-to-sequence (seq2seq) architecture~

\cite{wang2017tacotron}
...
framework, known through e.g. Google's Tacotron model. This framework has three
main parts: an encoder, an attention mechanism, and a decoder. The
...
the input text and converts it into a high-level representation. The attention
mechanism acts as a bridge, learning to align the text representation with the
output audio frames. At each step, it decides which part of the input text the
model should focus on to generate the next piece of audio. The decoder, also
typically an RNN, takes the encoder output and uses the attention context to
generate an acoustic representation of the speech, one frame at a time.

\subsubsection{Non-autoregressive (parallel) models}

FastPitch~\cite{lancucki2020fastpitch} is a non-autoregressive TTS model.
...
\textbf{The Inversion Problem:} Standard TTS models like Tacotron or FastPitch generate intermediate acoustic features (typically mel-spectrograms), but do not directly produce raw audio waveforms. However, mel-spectrograms are lossy representations that only capture the magnitude of the frequency bands, discarding phase information. This information is discarded during the STFT process. Thus, converting a spectrogram into audio is a non-trivial task, as the phase information must be estimated. This challenge is
known as the inversion problem.
...
\textbf{GAN-based Vocoders:} Neural GAN (Generative Adversarial Network) based vocoders, such as HiFi-GAN~\cite{kong2020hifi}, have become the dominant approach for vocoding in modern TTS systems. These GAN-based vocoder models consist of two main components: a Generator, which reconstructs mel-spectrograms into raw waveforms, and a Discriminator, which distinguishes between real and synthesized audio. This adversarial training forces the Generator to produce high-fidelity audio that closely resembles natural speech.

% TODO Why HiFi-GAN v2 is the chosen vocoder for this thesis.
...
% The relationship between training data size and model performance in neural
% networks follows predictable scaling laws. Research by Kaplan et
% al.~\cite{kaplan2020scaling} demonstrates that model performance generally
% improves as a power law function of the amount of training data, with
% diminishing returns at larger scales. These scaling laws provide a theoretical
% foundation for understanding why more training data typically leads to better
% TTS models, and why careful data selection becomes crucial when computational
% resources limit the amount of data that can be practically used for training.

% In the context of TTS systems, empirical studies have consistently shown that
% model quality improves with larger datasets. However, the marginal gains from
% additional data decrease as dataset size increases, making efficient data
% selection strategies increasingly valuable for achieving optimal
% performance-to-cost ratios.

% \subsubsection{Data Pruning}
% Data pruning focuses on removing samples that degrade model performance. In
% TTS, this often involves filtering out segments with high background noise,
% incorrect text transcriptions, or extreme silence. \cite{cooper2020zero}
% demonstrated that removing utterances with low MOS scores or alignment errors
% significantly improves synthesis stability.

% \subsubsection{Data selection strategies}

% \subsubsection{Active Learning and Coreset Selection}
% Active learning strategies attempt to select data points that the model is
% currently ``uncertain'' about, thereby maximizing the information gain of each
% training step. Alternatively, geometric approaches like Coreset selection aim
% to select a small subset of points that cover the geometric structure of the
% entire dataset in the embedding space.
...
% While modern multi-speaker TTS models are capable of high-fidelity synthesis, they typically rely on massive, curated datasets. There is limited research on data selection strategies: determining which subset of speakers or utterances yields the best model performance under a constrained data budget. Most current approaches blindly scale data size, leading to diminishing returns and high computational costs."

% While modern E2E architectures like FastPitch and HiFi-GAN have achieved near-human quality, they rely heavily on massive, high-quality datasets. In multi-speaker scenarios, the challenge is two-fold: ensuring acoustic consistency across diverse speakers and managing the computational cost of training on large corpora.

% Existing literature focuses heavily on model architecture (e.g., replacing RNNs with Transformers). However, there is less research on systematic data selection strategies - specifically, how to select the optimal subset of training data that maximizes multi-speaker performance while minimizing training time. This thesis addresses this gap by investigating [Your specific method]...

% % ----

% The evolution of TTS from mechanical devices to end-to-end neural networks has resulted in systems capable of near-human naturalness. However, current state-of-the-art multi-speaker models, such as FastPitch with HiFi-GAN, typically rely on massive, manually curated datasets.

% There is a gap in the literature regarding efficient \textbf{training data selection} strategies for these models. While scaling laws suggest that "more data is better," naive scaling often includes redundant or noisy data that hampers convergence. This thesis addresses how to algorithmically select the most informative subset of speakers and utterances to maximize TTS quality under computational constraints.
