...
\begin{itemize}
      \item \textbf{Speaker encoder model}: Pre-trained model from Coqui TTS based on
      ...
      \item \textbf{Embedding extraction}: Each speaker's audio samples are processed
            ...
            (d-vectors).
      \item \textbf{Pooling strategy}: Global average pooling across all utterances
            from each speaker to create a single representative embedding per speaker.
      \item \textbf{Normalization}: Speaker embeddings are L2-normalized to ensure
...
\begin{itemize}
      \item \textbf{Language filtering}: Only Lithuanian language samples are retained.
...
\textbf{Top-N Speaker Strategy (Speaker Depth):}
...
\textbf{Balanced Speaker Strategy (Speaker Breadth):}
...
\begin{itemize}
      ...
      \item \textbf{Test sentences:} A standardized set of 5 Lithuanian sentences
...
All experiments are implemented using the Coqui TTS framework~\cite{coqui2021},
