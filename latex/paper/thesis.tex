...
\begin{enumerate}
      ...
      \item \textbf{Attention mechanism:} A location-sensitive attention mechanism learns to align the high-level text representation with the decoder steps. This alignment determines which parts of the input text should be attended to when generating each of the output audio frames.
      ...
      \item \textbf{Stopnet:} A linear layer projects the decoder output to a scalar, predicting the probability that the current frame is the ``stop token'', which halts the synthesis process. This allows the model to dynamically determine the output duration.
\end{enumerate}
...
Firstly, although neural TTS models follow general scaling
